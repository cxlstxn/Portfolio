document.addEventListener("DOMContentLoaded", () => {
  ...
  const lsSections = ["scotbank", "arrancvs" ,"unixshell", "blackjackbot", "portfolio"];
  let history = [];
  let historyIndex = 0;
  const sections = {
  ...
  const introText =
    "Welcome to my portfolio! Type 'about', 'projects','contact' or 'help' to navigate.";
  let i = 0;
    ...
    if (e.key === "Enter") {
      ...
      if (command.startsWith("cd ")) {
        ...
        document.getElementById(
          "sec"
        ).textContent = `Current time: ${formattedTime}, Date: ${formattedDate}`;
        ...
        typewriterElement.textContent =
          "Invalid command. Try 'about', 'projects', or 'contact'.";
        setTimeout(() => {
          typewriterElement.textContent = introText;
        }, 2000);
      }
      ...
      if (parts[0] === "cd" && parts.length === 2) {
        const matches = lsSections.filter((section) =>
